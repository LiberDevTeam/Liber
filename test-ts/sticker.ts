...
contract('LiberSticker', (accounts) => {
  ...
  */
  ...
  it('should send a sticker', async () => {
    ...
    const targetEventLogs = transferEvent.logs.filter(
      (log) => log.event === 'TransferSingle'
    );
...
contract('LiberMarket', (accounts) => {
  ...
  it('should list token to market', async () => {
    ...
    const tokenId = async () => {
      ...
      const filteredEvents = events.logs.filter(
        (log) => log.event === 'PublishToken'
      );
    ...
    const approvedOperator = async () => {
      const events = await stickerInstance.setApprovalForAll(
        marketAddress,
        true
      );
      const filteredEvents = events.logs.filter(
        (log) => log.event === 'ApprovalForAll'
      );
      return filteredEvents.reduce((accm: string, log) => {
    ...
    assert.isOk(
      await approvedOperator(),
      'Approve to translate token for market'
    );

    const itemId = async () => {
      const events = await marketInstance.listToken(
        stickerAddress,
        await tokenId(),
        100,
        true
