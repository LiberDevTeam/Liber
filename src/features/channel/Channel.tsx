...
import { downloadTorrent, getClient, isNotTorrentFile, isTorrentFile, seed } from '../../torrent';
...
export default function ChannelPage() {
  ...
  const [progresses, setProgresses] = useState<Record<string, {progress: number, numPeers: number}>>({});
  ...
  function onFiles (files: FileList) {
    ...
    seed(fileAsArr.filter(isNotTorrentFile).map(file => {
      const result = /(?:(\.[^.]+))?$/.exec(file.name);
      return new File([file], `${nanoid()}${(result && result[1]) || ''}`, {type: file.type})
    }), onTorrent)
  }
  ...
  function handleSubmit() {
    ...
    const message = {id: uuidv4(), uid: me.id, text, timestamp: (new Date()).getTime()};
  ...
  useEffect(() => {
    ...
    messages.map((m) => {
      ...
      setProgresses(old => { return {...old, [m.id]: { progress: 0, numPeers: 0 }}})
      ...
      downloadTorrent((torrent) => {
        const updateProgress = (torrent: Torrent) => {
          setProgresses(old => { return {...old, [m.id]:{ progress: torrent.progress, numPeers: torrent.numPeers }}})
  ...
  return (
    <div className="h-screen pt-2 pb-4 overflow-y-scroll">
      <div className="px-2 py-2">
        <div className="">
          <h1 className="mt-2 mb-6 text-3xl font-semibold leading-6 text-gray-900">{channel.name || channel.id}</h1>
      ...
      <div className="sticky top-0 px-2 bg-gray-50 bg-opacity-25 pb-2">
        <input type="file" ref={fileRef} onChange={handleFileChanged} multiple />
        <div className="pt-0">
          <textarea ref={textareaRef} id="about" rows={2} className="p-2 shadow-lg focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border-gray-300 border" placeholder="Say Hello 👋"></textarea>
        </div>
        <button
          type="button"
        ...
        >
          Send
          <svg style={{transform: 'rotate(90deg)'}} className="ml-2 mr-1 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="px-2 text-grey-700">
        {messages.map((m) => {
          ...
          const Progress = () => {
            if (!m.attachment || !progresses[m.id] || progresses[m.id].progress === 1) return null;
            ...
            return (<div className="text-gray-500">Peers: {numPeers} Progress: {(100 * progress).toFixed(1)}% Download: {prettierBytes(downloadSpeed)}</div>);
          }
          return (
            <div key={m.id} className="my-3 px-1 pt-3 pb-4">
              <div className="mb-1">
                <span style={{ color: color, background: hex('#fff', color) < 2.2 ? '#333' : undefined }} className="mr-1">{name}</span>
                <span className="text-sm text-gray-400">| {format(fromUnixTime((Math.round(m.timestamp/1000))), 'yyyy-MM-dd HH:mm:ss')}</span>
              </div>
              <div>
                { m.text.split('\n').map(str => <p>{str}</p>) }
              </div>
