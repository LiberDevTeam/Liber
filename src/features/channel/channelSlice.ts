...
export const channelSlice = createSlice({
  ...
  initialState,
  reducers: {
    addMessage: (state, action: PayloadAction<{ cid: string, message: Message }>) => {
      const { cid, message } = action.payload;
      state.messages[cid] = [message].concat(state.messages[cid] || []);
      state.messagesIndex[message.id] = true;
    },
    setChannelMessages: (state, action: PayloadAction<{ cid: string, messages: Message[] }>) => {
      ...
      Object.values(messages).sort((a, b) => a.timestamp > b.timestamp ? -1 : 1);
      state.messages[cid] = messages;
      state.messagesIndex = messages.reduce((prev, message) => { return {...prev, [message.id]: true} }, {});
    },
  },
});
...
export const selectChannelMessages = (cid: string) => (state: RootState) => state.channel.messages[cid] || [];
